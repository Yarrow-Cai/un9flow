...
## 目标
- 作为 incident 辅助 skill，`主动识别证据缺口`，推动下一轮取证，最终形成可直接被 `incident-orchestrator` 消费的 `evidence-package`。
...
## 何时应使用
- 输入证据结构混乱、命名或时间轴不可直接消费。
- 当前证据不足，尚不足以进入 `incident` 主链（或下一轮 specialist 触发不足）。
- 某个 specialist 输出明确指出需要补取关键证据才能继续。
- 用户反馈当前信息碎片化、缺少关键截图/日志/波形/寄存器锚点时。
...
## 何时返回主场景
- 当证据结构可被 `incident-orchestrator` 直接消费，且缺口不再阻断下一步诊断时，返回 `incident-investigation`。
...
## Claude Code 宿主附录
- 当用户证据混乱、证据不足，或明确要求“先补证据”时，可直接进入本 skill。
